### Task 3: Create `pages/6_🔍_validation.py` — Validation Report Page

**Files:**
- Create: `pages/6_🔍_validation.py`

- [ ] **Step 1: Implement the validation page**
...
```bash
git add pages/6_🔍_validation.py
git commit -m "feat: add data validation page with three-tier report display"
```

---
### Task 4: Integrate Cleaning + Validation into Import Flow

**Files:**
- Modify: `pages/5_📥_import_data.py` (confirm-import button handler)

- [ ] **Step 1: Modify the "确认导入" button handler to run cleaning + validation**

In `pages/5_📥_import_data.py`, replace the confirm-import handler (the `if col1.button(...)` block around line 260) with:
...
```python
        ...
        if col1.button("✅ 确认导入，进入看板", type="primary", use_container_width=True):
            ...
            # Step 3: Block import if there are errors
...
Run: `cd "D:/cc项目/fund v2" && py -m pytest tests/test_data_cleaner.py tests/test_validator.py -v`
Expected: All tests PASS

- [ ] **Step 3: Commit**

```bash
git add pages/5_📥_import_data.py
git commit -m "feat: integrate cleaning + validation into import confirm flow"
```
